...
public class PictureRender implements GLSurfaceView.Renderer {
    ...
    public PictureRender(Context mContext) {
    ...
    void init(){
        vertex = "filter/half_color_vertex.sh";
        fragment = "filter/half_color_fragment.sh";

        ByteBuffer bb=ByteBuffer.allocateDirect(mPos.length*4);
        bb.order(ByteOrder.nativeOrder());
        bPos=bb.asFloatBuffer();
        bPos.put(mPos);
        bPos.position(0);
        ByteBuffer cc=ByteBuffer.allocateDirect(mCoord.length*4);
        cc.order(ByteOrder.nativeOrder());
        bCoord=cc.asFloatBuffer();
        bCoord.put(mCoord);
        bCoord.position(0);
        try {
            mBitmap = BitmapFactory.decodeStream(mContext.getAssets().open("shader/beautyLady.png"));
            GLES20.glClearColor(1.0f,1.0f,1.0f,1.0f);
        } catch (IOException e) {
            e.printStackTrace();
        }
        GLES20.glEnable(GLES20.GL_TEXTURE_2D);
        mProgram= GLUtils.createProgram(mContext.getResources(),vertex,fragment);
